...
function hotp(secretBytes: Uint8Array, counter: number, algorithm: string, digits: number): { code: string; binary: number; digest: Uint8Array } {
  const buffer = Buffer.alloc(8);
  buffer.writeBigUInt64BE(BigInt.asUintN(64, BigInt(counter)));
  const digest = new Uint8Array(createHmac(algorithm, buf(secretBytes)).update(buffer).digest());
...
const totpTool: ToolImpl = {
  ...
  async run(ctx): Promise<ToolResult> {
    ...
    const current = hotp(secretBytes, counter, algorithm, digits);
    ...
    if (mode === 'verify') {
      ...
      for (let offset = -window; offset <= window; offset += 1) {
        ...
        const computed = hotp(secretBytes, step, algorithm, digits).code;
    ...
    const blocks = [
      ...
      alignRows([
        ...
        [msg('sec.totp.row.26'), hotp(secretBytes, counter + 1, algorithm, digits).code],
        [msg('sec.totp.row.6'), `${localTime(uiLocale, at.ms)} · ${at.label}`],
        [msg('sec.totp.row.27'), `${counter} = floor(${nowSeconds} / ${period})`],
        [msg('common.label.secret'), msg('sec.totp.row.28', { secretBytes: encodeBase32(secretBytes), secretBytes2: secretBytes.length, g: groupedFingerprint(secretBytes, 'sha256').replace(/:/g, '') })],
        [msg('sec.totp.row.29'), `binary=${current.binary} → mod ${10 ** digits} → ${current.code}`],
      ]),
      section(msg('sec.totp.section.6', { window: window })),
      alignRows(
        Array.from({ length: 2 * window + 1 }, (_, index) => index - window).map((offset) => {
          const step = counter + offset;
          const value = step < 0 ? msg('sec.totp.row.30') : hotp(secretBytes, step, algorithm, digits).code;
      ...
      ...(digits === 8
        ...
        : [
            ...
            alignRows([
              [msg('sec.totp.row.31'), hotp(secretBytes, counter, algorithm, 8).code],
              [msg('sec.totp.row.32', { digits: digits }), msg('sec.totp.row.33', { digits: digits, digits2: hotp(secretBytes, counter, algorithm, 8).code.slice(8 - digits) })],
            ]),
          ]),
      section(msg('common.section.notes')),
      [
        ...
        msg('sec.totp.row.39', { GEZDGNBVGY3TQOJQGEZDGNBVGY3TQOJQ: encodeBase32(decodeBase32(msg, 'GEZDGNBVGY3TQOJQGEZDGNBVGY3TQOJQ')), code: hotp(decodeBase32(msg, 'GEZDGNBVGY3TQOJQGEZDGNBVGY3TQOJQ'), 1, 'sha1', 8).code }),
      ].join('\n'),
    ];
    await emitText(ctx, 'totp.txt', joinBlocks(blocks));
    ctx.report({ percent: 100, phase: 'done' });
...
function extensionRows(msg: Msg, cert: X509Certificate, pemBlock: string): Row[] {
  ...
  const infoAccess = (cert as unknown as { infoAccess?: string[] }).infoAccess ?? [];
...
function jsonOf(msg: Msg, cert: X509Certificate, pemBlock: string): Record<string, unknown> {
  ...
  return {
    ...
    subjectKeyIdentifier: extensions.subjectKeyId,
    authorityKeyIdentifier: extensions.authorityKeyId,
    infoAccess: (cert as unknown as { infoAccess?: string[] }).infoAccess ?? [],
...
function sampleFrom(chars: string): string {
  return chars.charAt(randomInt(0, chars.length));
}
...
function uuidV7(): { bytes: Uint8Array; ms: number } {
  ...
  if (ms <= v7State.ms) {
    ...
    v7State = { ms, seq: randomInt(0, 0x0fff) };
  }
